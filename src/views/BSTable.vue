...
export default {
  data() {
    return {
      ...
      items: [
        { age: 40, first_name: "Dickerson", last_name: "Macdonald", _rowVariant:'danger'   },
        { age: 21, first_name: "Larsen", last_name: "Shaw" , _rowVariant: ''},
        { age: 89, first_name: "Geneva", last_name: "Wilson", _rowVariant:'' },
      ],
      tableVariants: [
        "primary",
        "secondary",
  ...
  created(){
    setInterval(()=>{
        ...
        for(i;i<=this.items.length;i++){
            if(this.items[i]._rowVariant =='danger'){
                this.items[i]._rowVariant = ''
                let next = (i+1)%this.items.length
                this.items[next]._rowVariant = 'danger'
                break;
            }
